...
const OrderDetails = () => {
  ...
  const handleOutForDelivery = async () => {
    try {
      const response = await axios.post('http://localhost:4000/delivery/out-for-delivery', {
        orderId: order.order_id,
  ...
  const handleGenerateOtp = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:4000/delivery/generate-otp", {
  ...
  const handleVerifyDelivery = async () => {
    ...
    try {
      await axios.post("http://localhost:4000/delivery/verify-delivery", {
        orderId: order.order_id,
  ...
  const handleRejectOrderByUser = async () => {
    ...
    setLoading(true);
    try {
      await axios.post("http://localhost:4000/delivery/reject-order", {
        orderId: order.order_id,
        deliveryBoyId: userId, // Pass the delivery boy ID
        otp,
      });
  ...
  const handleRejectOrderNoResponse = async () => {
    setLoading(true);
    try {
      await axios.post("http://localhost:4000/delivery/reject-order-no-response", {
        orderId: order.order_id,
        deliveryBoyId: userId, // Pass the delivery boy ID
      });
  ...
  const handleRejectvender = async () => {
    setLoading(true);
    try {
      await axios.post("http://localhost:4000/delivery/reject-order-vender", {
        orderId: order.order_id,
        deliveryBoyId: userId, // Pass the delivery boy ID
      });
